...
{
    [SerializeField] private PortalOld[] portals = new PortalOld[2];
    ...
    private void UpdateCamera(ScriptableRenderContext SRC, Camera camera)
    {
        ...
        if (portals[0].Renderer.isVisible)
        {
            ...
            for (var i = iterations - 1; i >= 0; --i) RenderCamera(portals[0], portals[1], i, SRC);
        }

        if (portals[1].Renderer.isVisible)
        {
            portalCamera.targetTexture = tempTexture2;
            for (var i = iterations - 1; i >= 0; --i) RenderCamera(portals[1], portals[0], i, SRC);
        }
    }

    private void RenderCamera(PortalOld inPortal, PortalOld outPortal, int iterationID, ScriptableRenderContext SRC)
    {
        var inTransform = inPortal.transform;
        var outTransform = outPortal.transform;

        var cameraTransform = portalCamera.transform;
